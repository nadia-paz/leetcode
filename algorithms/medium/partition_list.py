...
#   +===================================================+
    ...
    def partition_list(self, x):
        ...
        # to move through the ll
        temp = self.head

        # create 2 linked list with dummy values
        list1 = LinkedList(None)
        list2 = LinkedList(None)
        # create pointers to those lists
        dummy1 = list1.head
        dummy2 = list2.head
        
        while temp:
            # create a new node with the value of each element
            new_node =  Node(temp.value)
            # assign to lists based on condition
            if temp.value < x:
                dummy1.next = new_node
                dummy1 = dummy1.next
            else:
                dummy2.next = new_node
                dummy2 = dummy2.next
                # print("Goes to dummy2")
            temp = temp.next
        

        # connect the list1 with the list 2 (don't add the 1st element of list 2)
        dummy1.next = list2.head.next

        self.make_empty()

        # reassign values in original ll, don't use the head of the 1st list
        self.head = list1.head.next
